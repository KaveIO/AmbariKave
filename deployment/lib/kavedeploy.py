...
def proxopts(portop='-p'):
    ...
    return [portop, str(proxy_port), '-o', 'UserKnownHostsFile=/dev/null',
            '-o', 'ConnectTimeout=600', '-o', 'StrictHostKeyChecking=no', '-o',
...
def strictopts():
    if strict_host_key_checking:
        return []
    return ['-o', 'UserKnownHostsFile=/dev/null', '-o', 'StrictHostKeyChecking=no', '-o', 'ConnectTimeout=600']
    ...
    def check(self, firsttime=False):
        ...
        if firsttime:
            extrasshopts = ["-o", "UserKnownHostsFile=/dev/null", "-o", "StrictHostKeyChecking=no", "-o",
                            "PasswordAuthentication=no", "-o", "ConnectTimeout=600"]
...
def wait_for_ambari(ambari, maxrounds=10, check_inst=None):
    ...
    while rounds <= maxrounds:
        ...
        try:
            ...
            disable_security(ambari, permanent=False)
        except ShellExecuteError:
            pass
